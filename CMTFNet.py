...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        res4 = self.attention(res4) 
        res3 = self.Conv2(res3)
        res3 = self.attention1(res3)
        x = self.b4(res4)
...
class ColorNet(nn.Module):
    def __init__(self,num_classes=2):
        ...
        self.bilinear = True
        self.inc1 = DoubleConv(self.num_classes, 64)
        self.down11 = Down(64, 128)
        self.down21 = Down(128, 256)
        self.down31 = Down(256, 512)
        factor = 2 if self.bilinear else 1
        self.down41 = Down(512, 1024 // factor)
        self.up11 = Up(1024, 512 // factor, self.bilinear)
        self.up21 = Up(512, 256 // factor, self.bilinear)
        self.up31 = Up(256, 128 // factor, self.bilinear)
        self.up41 = Up(128, 64, self.bilinear)
        self.outcy = OutConvy(64, 3)


    def forward(self, x):
        ##Transformer_branch

        y_input = torch.softmax(x,1)
        y1 = self.inc1(y_input)
        y2 = self.down11(y1)
        y3 = self.down21(y2)
        y4 = self.down31(y3)
        y5 = self.down41(y4)
